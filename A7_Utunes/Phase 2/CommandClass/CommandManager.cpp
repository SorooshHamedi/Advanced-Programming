...
void CommandManager::parseReadRequest(const InputLine& inputLine)
{
    ...
    else if(inputLine[COMMAND_IDENTIFIER_INDEX] == SIMILAR_USERS_COMMAND)
    {
        command = new GetSimilarUsersCommand(inputLine);
    }
    else if(inputLine[COMMAND_IDENTIFIER_INDEX] == RECOMMENDED_SONGS_COMMAND)
    {
        command = new GetRecommendedSongsCommand(inputLine);
    }
    else
    {
        notFoundError();
